var graph    = require("../index")
  , FBConfig = require("./config.2.0").facebook
  , vows     = require("vows")
...
vows.describe("graph.2.0.test").addBatch({
  ...
  "When accessing the graphApi": {
    "with no *Access Token** ": {
      ...
      "and searching for public data via id": {
        topic: function () {
          graph.get("/220439", this.callback)
        },

        "should be not able to get the user by their id": function (err, res) {
      ...
      "and searching for public data via username": {
        ...
        "should not be able to get a user by their username": function (err, res) {
          assert.include(res, "error");
          assert.equal(res.error.type, "OAuthException",
            "Response from facebook should be an OAuthException");
        }
      ...
      "and requesting a public profile picture using the user id": {
        topic: function () {
          graph.get("/4/picture", this.callback);
        },
    ...
    "with an *Access Token* ": {
      topic: function () {
        ...
        console.log(testUserUrl);
        console.log(testUserParams);
